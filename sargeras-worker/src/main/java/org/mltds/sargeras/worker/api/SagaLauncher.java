    ...
    private static void initBeanFactory() {
        try {
            ...
            for (Map.Entry e : entries) {
                ...
                Class<?> beanCls = getObject.getReturnType();
                SagaApplication.addBeanFactory(beanCls, beanFactory);// 根据 Bean 的类型，缓存 BeanFactory
                if (logger.isDebugEnabled()) {
                    logger.debug("初始化" + beanCls.getSimpleName() + "BeanFactory成功: " + factoryClass.getName());
                }
            }
        } catch (Throwable e) {
